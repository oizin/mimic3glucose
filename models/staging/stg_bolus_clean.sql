...
with bolus as (
    select SUBJECT_ID,HADM_ID,ICUSTAY_ID,STARTTIME,ENDTIME,AMOUNT,RATE,ORIGINALRATE,ITEMID,ORDERCATEGORYNAME,InsulinType,InsulinAdmin,INFXSTOP 
    from {{ ref('stg_insulin_raw') }}
    where upper(InsulinAdmin) in ('BOLUS_PUSH','BOLUS_INYECTION') and upper(InsulinType)='SHORT'
),
bolus99 as (
